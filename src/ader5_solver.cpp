// Fifth-order ADER solver: WENO9 (M=4) + CK order 4 + TT-GRP + 3-point Gauss.
    ...
    std::vector<Physical>& Fbar, std::vector<Conserved>& Unew)
{
    ...
    for (int f = 0; f < nx; ++f) {
        ...
        Physical fAder = ttGrpFluxAder5(ws.PolyL[f], ws.PolyR[f], dt, opt);
...
void ader5Advance(std::vector<Conserved>& U, double dx, double dt,
                  bool periodic, const TTGRPOptions& opt)
{
    const int nx = static_cast<int>(U.size());
    WENO1d weno(4, dx);
    ...
    ader5AdvanceFast(U, dx, dt, periodic, opt, weno, ws, Fbar, Unew);
}

void ader5Run(std::vector<Conserved>& U,
              ...
              bool periodic, const TTGRPOptions& opt)
{
    ...
    std::cout << "[ADER5] done: " << step << " steps, t=" << t << "\n";
}
